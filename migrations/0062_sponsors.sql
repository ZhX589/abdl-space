...
INSERT OR IGNORE INTO sponsor_settings(id, version, config_json) VALUES (1, 1, '{"enabled":false,"version":1,"center_title":"赞助者中心","free_daily_limit":10,"sponsor_daily_limit":100,"timezone":"Asia/Shanghai","notice_version":1,"notice_title":"查看原图须知","notice_body":"普通用户每天可查看 {x} 张原图，赞助者每天可查看 {y} 张，今日剩余 {a} 张，{reset} 重置。仅确认继续后记录本次查看。","exhausted_title":"今日原图额度已用完","exhausted_body":"普通用户每天可查看 {x} 张原图，将于 {reset} 重置。","sponsor_exhausted_body":"赞助者每天可查看 {y} 张原图，将于 {reset} 重置。","purchase_title":"赞助须知","purchase_steps":["请选择适合自己的赞助方案，理性赞助。","前往爱发电完成支付后，获取兑换码并返回赞助者中心兑换。","从购买页面返回不代表支付或兑换成功，请以赞助者中心显示为准。"],"minimum_read_seconds":5,"default_color_key":"blue","colors":[{"key":"blue","name":"蓝色","light":"#175CD3","dark":"#A6C8FF","permanent_only":false},{"key":"pink","name":"粉色","light":"#AD1457","dark":"#FFB1D0","permanent_only":false},{"key":"gold","name":"金色","light":"#795500","dark":"#F6D778","permanent_only":true}],"benefits":[{"id":"original","title":"更多原图额度","description":"身份有效期间自动提升每日原图额度。","status":"automatic","action":"original","sort_order":10},{"id":"color","title":"用户名颜色","description":"选择喜爱的用户名颜色，永久赞助者可选择金色。","status":"available","action":"color","sort_order":20},{"id":"lottery","title":"赞助者抽奖","description":"敬请期待，当前尚未开放。","status":"coming_soon","action":"none","sort_order":30},{"id":"more","title":"更多权益","description":"更多赞助者权益正在筹备。","status":"coming_soon","action":"none","sort_order":40}]}');
...
CREATE VIEW IF NOT EXISTS sponsor_user_state AS
SELECT u.id AS user_id,
  ...
  CASE WHEN m.permanent=1 OR m.expires_at>unixepoch() THEN COALESCE(m.color_key,json_extract(s.config_json,'$.default_color_key')) END AS color_key,
...
CREATE TRIGGER IF NOT EXISTS sponsor_operation_validate BEFORE INSERT ON sponsor_operations
...
BEGIN
  ...
  SELECT CASE WHEN NEW.kind='color' AND NOT EXISTS(SELECT 1 FROM sponsor_user_state v,json_each(v.config_json,'$.colors') c WHERE user_id=NEW.user_id AND json_extract(c.value,'$.key')=NEW.color_key AND (permanent=1 OR json_extract(c.value,'$.permanent_only')=0)) THEN RAISE(ABORT,'invalid_color') END;
